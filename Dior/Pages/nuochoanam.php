...
<head>
  <meta charset="utf-8">
  <title>Nước Hoa Nam - Shop Nước Hoa DA</title>
...
<body>
  ...
  <div class="site-content">
          ...
          <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
            ...
            <li class="nav-item dropdown">
              ...
              <ul class="dropdown-menu">
                <li><a class="dropdown-item active" href="nuochoanam.php">Nước hoa Nam</a></li>
    ...
    <section class="py-5 text-center text-white bg-opacity-75">
      <div class="container">
        <h1 class="fw-bold mb-3 text-shadow">Nước Hoa Nam</h1>
        <p class="lead">Thể hiện bản lĩnh, phong cách và sức hút riêng của phái mạnh.</p>
      </div>
    </section>
    ...
    <section class="container text-center py-5 bg-light bg-opacity-75 rounded-3">
      <div class="row g-4">
        ...
        $stmt = $pdo->prepare("
          ...
          WHERE (p.name LIKE '%Nam%' OR p.short_description LIKE '%nam%' OR p.description LIKE '%nam%')
          AND p.is_active = 1
          ORDER BY p.id DESC
        ");
        $stmt->execute();
          ...
          foreach ($products as $sp):
            ...
            if (!empty($sp['image_src'])) {
                if (strpos($sp['image_src'], 'Images/') === false) {
                    $img_path = '../Images/nuochoanam/' . ltrim($sp['image_src'], '/');
                } else {
